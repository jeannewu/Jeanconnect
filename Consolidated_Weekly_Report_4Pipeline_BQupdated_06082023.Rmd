...
for (i in 1: length(cnames)){
   ...
   recr_signin1[,cnames[i]] <- ifelse(numbers_only(var), as.numeric(as.character(var)), var)
     ...
     #saveRDS(recr_signinl1,paste("~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/recruit_signin",currentDate,".rds", sep=""))
   #recr_signin1 <- readRDS("~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/recruit_signin2023-06-05.rds" )
...
table43 <-  tbl(con,"participants_JP")  %>%  
  ...
  mutate(active_camptype = droplevels(factor(active_camptype, levels=c("Aging out of study","Demographic Group", 
                                                                       ...
                                                                       "Screening appointment", "Other", "None of these apply"))),
...
table43_Totalactivecamp <- table43 %>% select(Site, active_camptype) %>% 
  tbl_cross( row=Site,
             col = active_camptype,
...
table43_Totalactivecamp <- table43_Totalactivecamp %>%
  ...
  modify_column_hide(columns = stat_9) %>%
...
tmp <- as.data.frame(xtabs(~Site+Rcrt_Verified_v1r0+active_camptype,data=table43))
...
colnames(tmp_pct)[2:9] <- paste0(colnames(tmp_pct)[2:9],"_pct")
act.veri.camp <- merge(tmp_freq,tmp_pct,by="Site")

total.freq <- as.data.frame(t(apply(tmp_freq[,2:9],2,  sum)))
colnames(total.freq) <- colnames(tmp_freq)[2:9]
total.freq$Site <- "Total Active Recruits"

total.veri_pct <- pct_tb(table43$active_camptype, table43$Rcrt_Verified_v1r0)

total.freq0 <- cbind(total.freq,t(paste0(round(100*total.veri_pct$pct_353358909[1:8],2)," %")))
colnames(total.freq0)[10:ncol(total.freq0)] <- paste0(rownames(total.veri_pct)[1:8],"_pct")

campaign.combo <- rbind(act.veri.camp,total.freq0)
campaign.combo$`Total Active Recruits` <- Table3$Total.Active.Recruits
...
#Table 48. Daily Table 15 Age of All Records
 table59_ageall <- tbl(con,"participants_JP")  %>%  
  filter(.,(!is.na(d_827220437) & ((d_512820379 =='486306141' & d_821247024 != '922622075') | d_512820379 =='854703046' ))) %>% 
...
table60_raceall <- tbl(con,"participants_JP")  %>%  
  filter(.,(!is.na(d_827220437) & ((d_512820379 =='486306141' & d_821247024 != '922622075') | d_512820379 =='854703046' ))) %>% 
...
table61_sexall <- tbl(con,"participants_JP")  %>%  
  filter(.,(!is.na(d_827220437) & ((d_512820379 =='486306141' & d_821247024 != '922622075') | d_512820379 =='854703046' ))) %>% 
...
recr_resp <- tbl(con,"participants_JP")  %>%  
  ...
  mutate(recru_time = ymd_hms(d_471593703),
         ...
         recruit.week = ceiling(as.numeric(difftime(as_date(ymd_hms(d_471593703)), as_date(min(ymd_hms(d_471593703),na.rm=TRUE)),units="days"))/7),
         recrstart.Sunday = as_date(ifelse(wday(as.Date(min(ymd_hms(d_471593703),na.rm=TRUE))) == 1, as.Date(min(ymd_hms(d_471593703),na.rm=TRUE)),
         as_date(as.Date(min(ymd_hms(d_471593703),na.rm=TRUE))-wday(as.Date(min(ymd_hms(d_471593703),na.rm=TRUE))) +1 ))))
         
recr_resp$Week.Sunday <-  ceiling(as.numeric(difftime(as_date(ymd_hms(recr_resp$d_471593703)), as_date(recr_resp$recrstart.Sunday),units="days"))/7)      
...
veri_resp <- data[which(data$d_821247024 == 197316935 ),] %>%
  mutate(verified_time = ymd_hms(d_914594314),
         ...
         recrstart.date = as_date(min(as.POSIXct(d_471593703),na.rm=TRUE)),
         recrstart.Sunday = as_date(ifelse(wday(as.Date(min(ymd_hms(d_471593703),na.rm=TRUE))) == 1, as.Date(min(ymd_hms(d_471593703),na.rm=TRUE)),
                                           as_date(as.Date(min(ymd_hms(d_471593703),na.rm=TRUE))-wday(as.Date(min(ymd_hms(d_471593703),na.rm=TRUE))) +1 ))))

veri_resp$Week.Sunday <-  ceiling(as.numeric(difftime(as_date(as.POSIXct(veri_resp$recru_time)), as_date(veri_resp$recrstart.Sunday),units="days"))/7)      
...
veri_resp$Verified.Sunday <-  ceiling(as.numeric(difftime(as_date(as.POSIXct(veri_resp$verified_time)), as_date(veri_resp$recrstart.Sunday),units="days"))/7)      
...
recrui_wk_verified$Week.Sunday.date <- as_date(ifelse(is.na(recrui_wk_verified$Week.Sunday.date),recrui_wk_verified$Verified.Sunday.date,recrui_wk_verified$Week.Sunday.date))
...
for(site in unique(veri_resp[which(veri_resp$d_827220437>0),]$site)){
  ...
  subs_verif <- veri_resp[which(veri_resp$site == site & veri_resp$d_827220437>0 ),]%>% 
    group_by(Verified.Sunday,recrstart.date) %>%
...
veri_allsites$Verified.Sunday.date <- as_date(as.Date(veri_allsites$recrstart.date)-wday(as.Date(veri_allsites$recrstart.date)) +1 )+ dweeks(veri_allsites$Verified.Sunday)

max_y.site <- 100*ceiling(round(max(veri_allsites$Total.verified)/100,1))
breaks.site <- seq(300,max_y.site,300)

Fig2_verified_time <- ggplot(veri_allsites,aes(y=Total.verified, x=as.Date(Verified.Sunday.date, origin = lubridate::origin),color=site)) + geom_line() +    geom_point()+
...
veri_resp$eligible.Sunday <- ifelse(!is.na( veri_resp$d_130371375_d_266600170_d_787567527) ,ceiling(as.numeric(difftime(as_date(veri_resp$eligible.time),as_date(veri_resp$recrstart.Sunday),units="days"))/7),NA)
...
veri_wk_eligble.site$Verified.Sunday.date <- as_date(ifelse(is.na(veri_wk_eligble.site$Verified.Sunday.date), 
                                                    as_date(unique(veri_resp$recrstart.Sunday) + veri_wk_eligble.site$Verified.Sunday*7), veri_wk_eligble.site$Verified.Sunday.date))
...
# ##to create a data of the particpants with previous cancer history among those user profile complete monthly
...
# box_write(object = ca_hist[,c("token","Connect_ID","site", "age","sex", "race","Verification status","Recruitment type","User Profile submitted","Previous Cancer","Cancer Year","RcrtV_CStatusMatch_v1r0")], file_name=paste("prod_previous_cancer_cases_profile_complete_",currentDate,".csv",sep=""), row.names = F,na="")
...
rewd <- data[,c("token","Connect_ID", "site", "verified","d_827220437","recrt_steps_v1r0","d_821247024","d_512820379","d_912301837", unique(c(refwd.main1,refwd_type1)),refwd_date1,refwd_reasons)] 
#
...
for (i in 1:length(levels(refwd.cases.notes$site))) {
  ...
  site <- levels(refwd.cases.notes$site)[i]
...
write.csv(incentive[,c("token","site","NORC.Payment.Eligibility","Eligible.for.Payment","Time.Payment.Eligible.for.Round")],paste(outputpath,"Incentives_Weekly_Report_NORC",currentDate,".csv", sep=""),na="",row.names=F)

write.csv(incentive_nonchi[,c("token","site","NORC.Payment.Eligibility","Eligible.for.Payment","Time.Payment.Eligible.for.Round")],paste(outputpath,"Incentives_Weekly_Report_OTHERSITES_",currentDate,".csv", sep=""),na="",row.names=F)
...
#since the report will be checking the previous week till the past Sunday on each coming Monday
recruit.wk.active <- recr_resp[which(recr_resp$d_512820379==486306141 ),] %>% filter(.,as_date(recru_time) < currentDate) %>%
  group_by(Week.Sunday,Week.Sunday.date) %>%
  dplyr::summarize(active_recruits=n())

verified.wk.active <- veri_resp[which(veri_resp$d_821247024 == 197316935 & veri_resp$d_512820379==486306141),] %>%
  filter(.,as_date(recru_time) < currentDate) %>% 
  group_by(Verified.Sunday,Verified.Sunday.date) %>%
  dplyr::summarize(active_verifieds=n())
...
recrui_wk_verified <- merge(recruit.wk.active,verified.wk.all, by.x=c("Week.Sunday"),by.y=c("Verified.Sunday"),all.x=TRUE,all.y=TRUE)
recrui_wk_verified$Week.Sunday.date <- as_date(ifelse(is.na(recrui_wk_verified$Week.Sunday.date),recrui_wk_verified$Verified.Sunday.date,recrui_wk_verified$Week.Sunday.date))
recrui_wk_verified$Verified.Sunday.date <- as_date(ifelse(is.na(recrui_wk_verified$Verified.Sunday.date),recrui_wk_verified$Week.Sunday.date,recrui_wk_verified$Verified.Sunday.date))
...
##the activities in the verified participants
modules <- c("d_100767870","d_949302066","d_536735468","d_663265240","d_976570371","d_517311251","d_832139544","d_264644252","d_770257102")
##for biospecimen 
...
veri_act <- filter(veri_resp,d_821247024==197316935 & as_date(verified_time) < currentDate) %>% 
...
eligible.wk.total <- veri_act[which(veri_act$d_130371375_d_266600170_d_731498909 ==353358909 ),] %>%
  group_by(eligible.Sunday) %>%
  dplyr::summarize(total_eligible=n(),
                   eligible.time.max=max(eligible.time,rm.na=T))
...
veri_wk_eligble <- merge(verified.wk.total, eligible.wk.total,by.x="Verified.Sunday",by="eligible.Sunday", all.x=TRUE,all.y=TRUE)
veri_wk_eligble <- merge(veri_wk_eligble, module.wk.total,by.x="Verified.Sunday",by="module.week", all.x=TRUE,all.y=TRUE)
veri_wk_eligble <- merge(veri_wk_eligble, anybio.wk.total,by.x="Verified.Sunday",by="anybio.week", all.x=TRUE,all.y=TRUE)
veri_wk_eligble <- merge(veri_wk_eligble, biocol.wk.total,by.x="Verified.Sunday",by="biocol.week", all.x=TRUE,all.y=TRUE)
veri_wk_eligble <- merge(veri_wk_eligble, dtdestroy.wk.total,by.x="Verified.Sunday",by="dtdestroy.week", all.x=TRUE,all.y=TRUE)
veri_wk_eligble <- merge(veri_wk_eligble, wdconsent.wk.total,by.x="Verified.Sunday",by="wdconsent.week", all.x=TRUE,all.y=TRUE)
veri_wk_eligble <- merge(veri_wk_eligble, allacts.wk.total,by.x="Verified.Sunday",by="allacts.week", all.x=TRUE,all.y=TRUE)
veri_wk_eligble1 <- filter(veri_wk_eligble,Verified.Sunday.date>0) %>% 
  mutate(Verified.Sunday.date=as_date(ifelse(is.na(Verified.Sunday.date),as_date(recrstart.date)+dweeks(Verified.Sunday-1)+days(5),Verified.Sunday.date)))


###mutate_at(vars(match())) cannot work on here with an error messages
vars <- c("total_verifieds","total_module","total_anybio","total_eligible","total_biocol","total_dtdestroy","total_wdconsent","total_allacts")
recr_veri_wk <-  merge(recrui_wk_verified,veri_wk_eligble1, by.x=c("total_verifieds","Week.Sunday","Week.Sunday.date"),by.y=c("total_verifieds","Verified.Sunday","Verified.Sunday.date"),all.x=TRUE,all.y=TRUE)
#recr_veri_wk <- recr_veri_wk %>% dplyr::select(.,-c("total_verifieds.y")) 

recr_veri_wk <- recr_veri_wk %>% mutate_at(vars(c("total_eligible","total_module","total_anybio","total_biocol","total_dtdestroy","total_wdconsent","total_allacts")),~replace_na(., 0)) %>%  arrange(Week.Sunday) %>% 
...
recr_veri_wk$Week.Starting <- recr_veri_wk$Week.Sunday.date-days(6)
recr_veri_wk$Week.Ending <- recr_veri_wk$Week.Sunday.date
recr_veri_wk$week.day <- wday(recr_veri_wk$Week.Starting,label=TRUE,abbr=F)
recr_veri_wk <- recr_veri_wk %>% mutate(total_analysis=cumsum(total_verifieds)-cumsum(total_dtdestroy),
...
var.select <- c(  "active_recruits.cum", "passive_verifieds.cum", "total_verified.cum", "cum.total.response.rate", 
                  "active_recruits", "total_verifieds", "total_anybio.cum","anybio_Invites_ratio","anybio_Verifieds_ratio",
...
base <- recr_veri_wk[,c("Week.Sunday", "Week.Starting", "Week.Ending","week.day")] #"Cumulative Revocations", "Revocation/ Verified", "Revocations per week", 
...
write.csv(base[which(base$Week.Starting < as.Date(currentDate) & base$Week.Ending < as.Date(currentDate)),],paste("~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/CCC_metrics_weekly_log_labels_",currentDate,".csv",sep=""),na="",row.names=F)
...
recruit.wk.active.site <- recr_resp[which(recr_resp$d_512820379==486306141 & as_date(recr_resp$recru_time) < currentDate),] %>%      arrange(Site,Week.Sunday) %>% group_by(Site,Week.Sunday,Week.Sunday.date) %>%
  dplyr::summarize(active_recruits=n(),
                   recruitdate_max=max(recru_time,rm.na=T))

verified.wk.active.site <- veri_act[which(as_date(veri_act$verified_time) < currentDate & veri_act$d_512820379==486306141),] %>%  arrange(site,Verified.Sunday) %>% group_by(site,Verified.Sunday,Verified.Sunday.date) %>%
  dplyr::summarize(active_verifieds=n(),
                   verified_activetime.max=max(verified_time,rm.na=T))

verified.wk.total.site <- veri_act[which(as_date(veri_act$verified_time) < currentDate & veri_act$d_821247024 == 197316935),] %>% arrange(site,Verified.Sunday) %>% 
  group_by(site,Verified.Sunday,Verified.Sunday.date) %>%
  dplyr::summarize(total_verifieds=n(),
...
###verified group
eligible.wk.total.site <- veri_act %>% 
  filter(., eligible.time < currentDate & d_130371375_d_266600170_d_731498909 ==353358909 ) %>%  
  group_by(site,eligible.Sunday) %>%
  dplyr::summarize(total_eligible=n(),
                   eligible.time.max=max(eligible.time,rm.na=T))

module.wk.total.site <- filter(veri_act,d_100767870==353358909 & module.time < currentDate) %>% arrange(site,module.week,verified.week)  %>%
  group_by(site,module.week)%>%
  dplyr::summarize(total_module=n(),
                   module.time.max=max(as_date(module.time),rm.na=T))

anybio.wk.total.site <- filter(veri_act,biospeDonation!= "No Sample Donations" & bioany.time < currentDate) %>% arrange(site,anybio.week,verified.week) %>% 
...
biocol.wk.total.site <- filter(veri_act, (biospeDonation =="Completed All 3 Sample Donations" | 
                                            (d_173836415_d_266600170_d_592099155==664882224 & d_878865966 == 353358909 & d_167958071 == 353358909) & biocol.time < currentDate))%>% 
  arrange(site,biocol.week,verified.week) %>% 
  group_by(site,biocol.week)%>%
  dplyr::summarize(total_biocol=n(),
                   biocol.time.max=max(biocol.time,rm.na=T))

allacts.wk.total.site <- filter(veri_act,allacts==353358909 & allacts.time < currentDate) %>% arrange(site,allacts.week,verified.week) %>% 
...
veri_wk_eligble.site <- merge(verified.wk.total.site, eligible.wk.total.site,by.x=c("site","Verified.Sunday"),by=c("site","eligible.Sunday"), all.x=TRUE,all.y=TRUE)
veri_wk_eligble.site <- merge(veri_wk_eligble.site, module.wk.total.site,by.x=c("site","Verified.Sunday"),by=c("site","module.week"), all.x=TRUE,all.y=TRUE)
veri_wk_eligble.site <- merge(veri_wk_eligble.site,anybio.wk.total.site,by.x=c("site","Verified.Sunday"),by=c("site","anybio.week"), all.x=TRUE,all.y=TRUE)
veri_wk_eligble.site <- merge(veri_wk_eligble.site, biocol.wk.total.site,by.x=c("site","Verified.Sunday"),by=c("site","biocol.week"), all.x=TRUE,all.y=TRUE)
veri_wk_eligble.site <- merge(veri_wk_eligble.site, dtdestroy.wk.total.site,by.x=c("site","Verified.Sunday"),by=c("site","dtdestroy.week"), all.x=TRUE,all.y=TRUE)
veri_wk_eligble.site <- merge(veri_wk_eligble.site, wdconsent.wk.total.site,by.x=c("site","Verified.Sunday"),by=c("site","wdconsent.week"), all.x=TRUE,all.y=TRUE)
veri_wk_eligble.site <- merge(veri_wk_eligble.site, allacts.wk.total.site,by.x=c("site","Verified.Sunday"),by=c("site","allacts.week"), all.x=TRUE,all.y=TRUE)

#veri_wk_eligble.site <- veri_wk_eligble.site %>% mutate_at(vars(matches("total")),~replace_na(., 0))

recr_veri_wk.site <-  merge(recrui_wk_verified.site,veri_wk_eligble.site, by.x=c("Site","Week.Sunday", "verified_time.max","total_verifieds"),by.y=c("site","Verified.Sunday", "verified_time.max","total_verifieds"),all.x=TRUE,all.y=TRUE)
...
recr_veri_wk.site$Week.Sunday.date <- as_date(ifelse(is.na(recr_veri_wk.site$Week.Sunday.date), recrstart.date + days(2)+dweeks(recr_veri_wk.site$Week.Sunday-1),recr_veri_wk.site$Week.Sunday.date))
recr_veri_wk.site$Week.Starting <- recr_veri_wk.site$Week.Sunday.date - days(6)
recr_veri_wk.site$week.day <- wday(recr_veri_wk.site$Week.Starting,label=TRUE,abbr=F)
recr_veri_wk.site$Week.Ending <- recr_veri_wk.site$Week.Sunday.date
...
for(i in 1:length(Site)){
  ...
  subs.recrui_verified <- subs.recrui_verified %>% arrange(Week.Sunday,Week.Sunday.date)
  ...
  subs.recr_veri_wk <- subs.recrui_verified %>% arrange(Week.Sunday)%>% 
  ...
  sub.base <- subs.recr_veri_wk[,c("Site","Week.Sunday", "Week.Starting", "Week.Ending", "week.day")] #"Cumulative Revocations", "Revocation/ Verified", "Revocations per week", 
  ...
  write.csv(sub.base[which(sub.base$Week.Starting < as.Date(currentDate) & sub.base$Week.Ending < as.Date(currentDate)),],paste(outputpath,"CCC_metrics_weekly_log_labels_",Site[i],currentDate,".csv", sep=""),na="",row.names=F)
  ...
  site_recruit_verified[[i]] <- sub.base
  
  write.csv(sub.base,paste(outputpath,"CCC_metrics_weekly_log",Site[i],currentDate,".csv", sep=""),na="",row.names=F)
  
}  

```

```{r time, timeit = TRUE,results='hold'}
ending.time <-Sys.time()
processing.time <- difftime(ending.time, starting.time,units="mins")
processing.time
```
