...
var.list <- unique(c("d_512820379",rfact_ls,bio.col,clc.bldtm,clc.urinetm,incentive.vars,modules,recruit.times,ref.wd,ca.hist.var,
                     refwd_date1,refwd_type1,refwd_reasons,refwd.main1,"d_821247024","d_230663853","d_919254129","d_699625233",
...
tb3_a <- tbl(con,"participants_JP") %>% filter(., d_512820379 != '180583933' | d_821247024 =='197316935') %>% 
  ...
  mutate(site = factor(site, levels=c("HealthPartners", "Henry Ford Health System","Marshfield Clinic Health System",
                                      "Sanford Health", "University of Chicago Medicine","Kaiser Permanente Colorado",
                                      "Kaiser Permanente Georgia","Kaiser Permanente Hawaii","Kaiser Permanente Northwest"))) %>% 
  group_by(site, recrtype_verified) %>% tally() %>% as_tibble()

verified <- dcast(tb3_a[which(!is.na(tb3_a$recrtype_verified)),], site ~ recrtype_verified, value.var = "n")
colnames(verified)[1] <- "Site"

verified <- verified %>% mutate(Total.Active.Recruits = Active_verified + `Active But Not Verified`)
verified.all <-  as.data.frame(t(colSums(Filter(is.numeric, verified)))) 
verified.all$Site <- "Total"
Verified <- rbind(verified,verified.all)

Table3 <- Verified %>% mutate(Total_Verified = Active_verified + Passive_verified,
                              across((contains("_verified")), ~col_pct(.x, Total.Active.Recruits)))
...
ca.var.hist <- colnames(data)[grepl("266952173|452166062|494982282|650597106|547895941",colnames(data))]
  ...
  ca_hist <- dt %>% filter(.,d_699625233=='353358909' & d_452166062 == '353358909' ) %>% select(token,Connect_ID,d_821247024, d_512820379, Site, age,sex, race,d_699625233,d_452166062,all_of(ca.var.hist)) %>%mutate(`Verification status`= case_when(d_821247024 == '875007964' ~ "Not yet verified",
                                             d_821247024 == '197316935'  ~ "Verified",
                                             d_821247024 ==  '219863910' ~ "Cannot be verified",
                                             d_821247024 ==  '922622075' ~ "Duplicate",
                                             d_821247024 ==  '160161595' ~ "Outreach timed out"),
...
optout <- tbl(con,"participants_JP")  %>%  
  filter(.,d_512820379=='486306141' & !is.na(d_827220437) & d_919254129 %in% c('104430631',NA)) %>% 
...
#Table 45
optout_pct <- optout2 %>% mutate(`Total Opt-Outs` = ifelse(is.na(`Total Opt-Outs`),0, `Total Opt-Outs`),
                                 Percent = ifelse(is.na(`Total Opt-Outs`),"0 %",paste0(round(100*`Total Opt-Outs`/total,2)," %")),
...
opt_out <- tbl(con,"participants_JP")  %>%  
  filter(.,d_512820379=='486306141' & !is.na(d_827220437) & state_d_158291096 =='353358909' & d_919254129 %in% c('104430631',NA)) %>% 
...
optouts1_all <- merge(optouts_all,optout_pct[,c("Site","Total Opt-Outs","order")],by.x="Site",by.y="Site", all.y=TRUE)
...
veri_resp <- data[which(data$d_821247024 == 197316935 ),time.list] %>%
...
knitr::kable(Table3[,c(1,3,4,6,5)],format.args = list(big.mark = ","),  caption='Table 3. Verified Participants and Response Ratio based on Number of Active Recruits', row.names=FALSE, col.names=c("Site", "Active Response", "Passive Response","Total Response", "Total Active  Recruits"),align=c("l","c", "c", "c","c")) %>% add_indent(c(1:9))
...
###verified group
eligible.wk.total.site <- veri_act[which(veri_act$eligible.time < currentDate & veri_resp$d_130371375_d_266600170_d_731498909 ==353358909 ),] %>% 
