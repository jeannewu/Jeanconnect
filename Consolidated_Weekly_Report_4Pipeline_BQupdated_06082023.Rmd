...
# [1] "state_state_d_706283025_d_517101990"      checked on 06/07/2023 shown in the BQ participants_JK table
 optout1 <- opt_out[!empty_columns] #32 columns #34 columns(02/21/2023) #35 columns(05/30/2023)         
...
plot.data <- recr_veri_act %>%
    gather(refwd_invite,value, total_dtdestroy,total_wdconsent #,new.refwd.rate
           ) %>% select(Week.Sunday,Week.Sunday.date,active_recruits,refwd_invite,value) %>% 
...
##refusal & data destruction
 ...
 break2.s <- seq(0,max_ys,1)
   ...
   plot.data.site <- filter(recrui_wk_verified1.site,site==s & as.Date(Week.Sunday.date) < currentDate) %>% 
     gather(refwd_invite,value, total_dtdestroy,total_wdconsent) %>% select(Week.Sunday,Week.Sunday.date,active_recruits,refwd_invite,value) %>% 
  mutate(type=case_when(#refwd_invite=="new.refwd.rate" ~"# withdrawal per 1K invitations",
                        refwd_invite=="total_wdconsent" ~"Withdrawals Only",
                        refwd_invite=="total_dtdestroy" ~"Data Destruction Only"),
         type = factor(type,levels=c("Withdrawals Only","Data Destruction Only")),
         value.new = ifelse(refwd_invite=="new.refwd.rate", value/a, value)) %>% 
ggplot(aes(y=value.new,x=as.Date(Week.Sunday.date),color=type)) + 
  geom_line() +  geom_point() + scale_color_manual(values = c("gold","red"), 
                     name="" ,labels=c("Withdrawals Only","Data Destruction Only"))+
  scale_y_continuous(name="# of Withdrawals And \nData Destruction Qequests", limits=c(0,max_ys),breaks=break2.s, #sec.axis = sec_axis(~ .*a, name = "# withdrawal per 1K invitations",breaks=break2.s)
                     ) +     
...
# We would like 3 of these plots total, one plot for all sites combined and then 2 additional plots for 2 groups of sites. Group 1 would be the KPs and University of Chicago. Group 2 would be the remaining four sites, HP, HFH, SF, MF.
 ...
 max_y.refwd <- 10*ceiling(max(recr_veri_act$total_wdconsent.cum)/10)
 break.rfwd <- seq(0,max_y.refwd,ceiling(max(recr_veri_act$total_wdconsent.cum)/10))
 rfwd.cum.plot <- recr_veri_act %>% 
   gather(refwd_type,value.cum, total_dtdestroy.cum,total_wdconsent.cum) %>% 
           select(Week.Sunday,Week.Sunday.date,active_recruits,refwd_type,value.cum) %>% 
  mutate(type=case_when(#refwd_invite=="new.refwd.rate" ~"# Withdrawal per 1K invitations",
                        refwd_type=="total_wdconsent.cum" ~"Withdrawals Only",
                        refwd_type=="total_dtdestroy.cum" ~"Data Destruction Only"),
         type = factor(type,levels=c("Withdrawals Only","Data Destruction Only"))) %>% 
ggplot(aes(y=value.cum,x=as.Date(Week.Sunday.date),color=type)) + 
  geom_line() +  geom_point() + scale_color_manual(values = c("darkorange","darkred"), name="",
                     #name="Withdrawals And Data Destruction" ,
                     labels=c("Withdrawals Only","Data Destruction Only"))+
  scale_y_continuous(name="# of Withdrawals And \nData Destruction Qequests",limits=c(0,max_y.refwd),breaks=break.rfwd)+ #limits=c(0,10*round(max(recr_veri_act$total_wdconsent/10),0)),breaks=break2,sec.axis = sec_axis( ~ . /2  , name = "# Withdrawal per 1K invitations",breaks=break2/2)
                      
  coord_cartesian(ylim =c(0,max_y.refwd)) +
  scale_x_date(name="Date",limit=as.Date(c("2021-12-31",currentDate)),date_labels = "%y-%m-%d", date_breaks = "2 weeks",expand=c(0,0))  +
  ggtitle(paste("Cumulative Withdrawals and Data Destructions \nby Number of Verified Participants Through", currentDate,"Overall",sep=" "))+
  theme( axis.text.x=element_text(angle=60, hjust=1), axis.title.x = element_text( size = 14, face = "bold") ,
         axis.title.y = element_text(size = 14, face = "bold",color="black",angle=90),
         plot.title = element_text(face="bold", size=16),
         # Remove panel border
 ...
 ##non KP sites & KP sites:
 recrui_wk_verified1.kp <- recrui_wk_verified1.site %>% group_by(KPgroups,Week.Sunday,Week.Sunday.date) %>% dplyr::summarize(total_dtdestroy=sum(total_dtdestroy),
                 total_wdconsent=sum(total_wdconsent),
                 total_verifieds=sum(total_verifieds)) %>% group_by(KPgroups) %>% 
...
for (KP in c(0,1)){
  dt <- recrui_wk_verified1.kp %>% filter(., KPgroups==KP)
  note <- if(KP==1){"KPs & UChicago"}
            else{
             "HP, HFH, SF, MF" 
            }
#print(note)
 max_y.refwd.kp <- 10*ceiling(max(dt$total_wdconsent.cum.kp)/10)
 break.rfwd.kp <- seq(ceiling(max(dt$total_wdconsent.cum.kp)/10),max_y.refwd,ceiling(max(dt$total_wdconsent.cum.kp)/10))
 rfwd.cum.plot.kp <- dt %>%  gather(refwd_type,value.cum, total_dtdestroy.cum.kp,total_wdconsent.cum.kp) %>% 
           select(Week.Sunday,Week.Sunday.date,refwd_type,value.cum) %>% 
  mutate(type=case_when(#refwd_invite=="new.refwd.rate" ~"# Withdrawal per 1K invitations",
                        refwd_type=="total_wdconsent.cum.kp" ~"Withdrawals Only",
                        refwd_type=="total_dtdestroy.cum.kp" ~"Data Destruction Only"),
         type = factor(type,levels=c("Withdrawals Only","Data Destruction Only"))) %>% 
ggplot(aes(y=value.cum,x=as.Date(Week.Sunday.date),color=type)) + 
  geom_line() +  geom_point() + scale_color_manual(values = c("darkorange","darkred"), name="",
                     #name="Withdrawals And Data Destruction" ,
                     labels=c("Withdrawals Only","Data Destruction Only"))+
  scale_y_continuous(name="# of Withdrawals And \nData Destruction Qequests", limits=c(0,max_y.refwd.kp),
                     breaks=break.rfwd.kp) +
   coord_cartesian(ylim =c(0,max_y.refwd.kp)) +
  scale_x_date(name="Date",limit=as.Date(c("2021-12-31",currentDate)),date_labels = "%y-%m-%d", date_breaks = "2 weeks",expand=c(0,0))  +
  ggtitle(paste("Cumulative Withdrawals and Data Destructions \nby Number of Verified Participants Through", currentDate,note,sep=" "))+
  theme( axis.text.x=element_text(angle=60, hjust=1), axis.title.x = element_text( size = 14, face = "bold") ,
         axis.title.y = element_text(size = 14, face = "bold",color="black",angle=90),
         plot.title = element_text(face="bold", size=16),
         # Remove panel border
...
```{r, eval=TRUE,echo=FALSE,fig.cap='Figures 5.11',results='asis'}
 for (KP in c(0,1)){
   dt <- recrui_wk_verified1.kp %>% filter(., KPgroups==KP)
   note <- if(KP==1){"KPs & UChicago"}
             else{
  ...
  max_y.refwd.kp <- 10*ceiling(max(dt$total_wdconsent.cum.kp)/10)
  break.rfwd.kp <- seq(ceiling(max(dt$total_wdconsent.cum.kp)/10),max_y.refwd,ceiling(max(dt$total_wdconsent.cum.kp)/10))
  rfwd.cum.plot.kp <- filter(dt,as.Date(Week.Sunday.date) < currentDate) %>%  gather(refwd_type,value.cum, total_dtdestroy.cum.kp,total_wdconsent.cum.kp) %>% 
            select(Week.Sunday,Week.Sunday.date,refwd_type,value.cum) %>% 
   mutate(type=case_when(#refwd_invite=="new.refwd.rate" ~"# Withdrawal per 1K invitations",
                         refwd_type=="total_wdconsent.cum.kp" ~"Withdrawals Only",
                         refwd_type=="total_dtdestroy.cum.kp" ~"Data Destruction Only"),
 ...
 ggplot(aes(y=value.cum,x=as.Date(Week.Sunday.date),color=type)) + 
   ...
   scale_y_continuous(name="# of Withdrawals And \nData Destruction Qequests", limits=c(0,max_y.refwd.kp),
    ...
    coord_cartesian(ylim =c(0,max_y.refwd.kp)) +
   scale_x_date(name="Date",limit=as.Date(c("2021-12-31",currentDate)),date_labels = "%y-%m-%d", date_breaks = "2 weeks",expand=c(0,0))  +
   ggtitle(paste("Cumulative Withdrawals and Data Destructions \nby Number of Verified Participants \nThrough", currentDate,note,sep=" "))+
   theme( axis.text.x=element_text(angle=60, hjust=1), axis.title.x = element_text( size = 14, face = "bold") ,
          axis.title.y = element_text(size = 14, face = "bold",color="black",angle=90),
          plot.title = element_text(face="bold", size=16),
          # Remove panel border
