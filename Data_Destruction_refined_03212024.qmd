...
outputpath <-"~/Documents/Connect_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/working_records/DataDestruction/Routputs/"
...
dd.y[grepl("919699172",y$conceptId.3),c("conceptId.3","Variable.Label","Current.Source.Question")]
...
recrvar <- recrvar %>% 
    mutate(stub_var = ifelse(grepl("state_studyId",field_path), 1, ifelse(grepl(paste(all.selected,collapse = "|"),field_path), 1,0)),
           ...
           stub_cid_130371375 =ifelse(grepl("130371375",field_path) & grepl(paste(all.selected,collapse = "|"),field_path), str_split_i(field_path,"_",-1),NA)) #152
...
write.csv(check1_stubs, paste0(outputpath, "Connect_DEV_Data_Destructions_flattened_Summary_Connect_5810010461_",Sys.Date(),".csv",na=""),row.names=F)
    ...
    check2 <- fun.query_id("FlatConnect","participants_JP","*",idss)
    check2_stubs <- check1 %>% select(contains(all.selected))
