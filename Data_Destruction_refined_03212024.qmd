...
stub_var_part.prod <- filter(tmp, grepl(paste(all.selected,collapse="|"),tmp$field_path))
...
stubs_var_participants <- filter(tmp, grepl(paste(all.selected, collapse="|"), column_name))
#I did check the participants tables in both stg and prod: the total variables are different.
stubs_var_participants_prod <- filter(tmp, grepl(paste(all.selected, collapse="|"), column_name)) #186
