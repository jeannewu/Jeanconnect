...
month <- sapply(strsplit(as.character(currentDate),"-"),"[")[2]
...
birthday_card <- filter(birth2,d_564964481 == month & !grepl("Chicago",Site)) %>% select(Connect_Id,"Last Name","First Name","Address Line 1",`Address Line 2`,`City`,`State`,`Zip`)

table(birth2$Site,birth2$d_564964481)

write.csv("NORC_BirthdayCard_",currentDate,".csv")

month = sapply(strsplit(format(Sys.Date(), "%Y %B %b %m"), " "),"[")[2]
...
box_write(object =birthday_card,file_name=paste("~/Documents/Connect_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/CSV_file_for_",month,"_Birthday_Card_Recipients_2024.csv",sep=""),na="")
